...
def test_attack_path_api_enforces_authentication_and_returns_analysis() -> None:
    ...
    unauthenticated = client.post("/api/v1/attack-paths/analyze", json=payload)
    assert unauthenticated.status_code == 401

    principal = Principal(subject="analyst-1", roles=frozenset({"analyst"}), tenant_ids=frozenset({"tenant-a"}))
    app.dependency_overrides[require_authenticated_analyst] = lambda: principal
    try:
        authorized = client.post("/api/v1/attack-paths/analyze", json=payload)
    finally:
        app.dependency_overrides.clear()

    assert authorized.status_code == 200
    assert authorized.json()["ranked_paths"][0]["risk_level"] == "critical"
    assert authorized.json()["tenant_id"] == "tenant-a"
