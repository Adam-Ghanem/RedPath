...
from app.core.auth import require_siem_reader
...
def test_siem_reader_binds_bearer_token_to_allow_listed_tenant() -> None:
    settings = Settings(siem_ingestion_api_token="reader-token", siem_allowed_tenants="lab,tenant-2")
    dependency = require_siem_reader(settings)
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="reader-token")

    principal = dependency(credentials=credentials, tenant_header="tenant-2")

    assert principal.actor == "siem-reader"
    assert principal.role == "siem_reader"
    assert principal.tenant_id == "tenant-2"
    with pytest.raises(HTTPException) as exc_info:
        dependency(credentials=credentials, tenant_header="tenant-3")
...
def test_protected_api_ingest_and_readback(tmp_path, monkeypatch) -> None:
    ...
    settings = Settings(
        ...
        siem_ingestion_api_token="reader-token",
        siem_allowed_tenants="lab",
    )
    app = FastAPI()
    ...
    with TestClient(app) as client:
        ...
        headers = {"Authorization": "Bearer reader-token", "X-RedPath-Tenant": "lab"}
