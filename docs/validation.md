...
| Backend tests | `PYTHONPATH=backend pytest -q` | Passed: 13 tests |
...
The enterprise increment additionally validates chained audit verification, canonical evidence manifests, remediation SLA classification, and deterministic campaign export packages. These controls are read-only with respect to external lab systems and do not persist credentials or private signing keys.
