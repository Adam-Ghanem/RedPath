...
| POST | `/api/v1/evidence/{evidence_id}/integrity/reverify` | Re-verify the immutable manifest and metadata-only storage state | `view_audit` permission; server-derived actor; fails closed on mismatch |
| POST/GET | `/api/v1/evidence/{evidence_id}/legal-hold` | Place, release, or inspect a legal hold | `manage_cases` for changes; `read` for inspection; tenant-filtered |
| POST/GET | `/api/v1/evidence/{evidence_id}/retention-decision` and `/retention-history` | Record and review append-only retention decisions | Bounded history; reason is redacted; actor is server-derived |
| POST | `/api/v1/evidence/{evidence_id}/deletion-request` | Request evidence metadata cleanup review | Requires eligible retention and no active hold; never deletes evidence |
| POST | `/api/v1/evidence/{evidence_id}/deletion-request/{request_id}/decision` | Approve or reject a deletion request | `view_audit`; independent approver required; safe 409 on failed gates |
| GET | `/api/v1/evidence/{evidence_id}/privacy-summary` | Return a bounded privacy-safe governance summary | No raw packets, payloads, credentials, notes, or storage locators |
...
Completed and failed jobs are retained only within the configured retention window and maximum history count per tenant. Retention deletes job metadata, not normalized assets or audit records. Active queued/running jobs are not evicted by count-based pruning. `GET /api/v1/inventory/assets/page` uses a bounded numeric cursor and a maximum page size of 100; search text is capped, and audit events store a digest of filters rather than the submitted terms. Workers acquire a tenant-scoped lease, persist only bounded checkpoint stages, and release the lease on completion or safe failure. Transient timeout/connection failures are classified against a small retry budget and require an explicit local retry; permanent or exhausted failures are not retried. Job results omit command argument arrays and are compacted under the configured byte budget. Stale running jobs are failed after the bounded recovery window and emit a recovery audit event rather than retrying network actions. Inventory is read-only and never executes scanning or mutates external systems.
...
`POST /api/v1/risk/ai-assess` and `POST /api/v1/copilot/explain` require an authenticated principal with the `analyze` permission. Clients send only `finding_id` for findings, or `analysis_id` plus `path_id` for registered attack paths. The tenant and actor are derived from the bearer session; findings and registered analyses are loaded with a tenant predicate, and absent or cross-tenant sources return the platform’s generic not-found response. Scores, tiers, centrality, linked evidence, and rationale are never accepted from the client.

`AI_FEATURES_ENABLED` defaults to `false`. Disabled, unavailable, timed-out, rate-limited, or invalid provider responses return the deterministic score and tier with an explicit fallback reason, so the risk endpoint remains available without model availability. When enabled, the external provider receives only minimized deterministic scores, centrality, counts, technique IDs, severity, and categorical evidence-basis tokens. Direct asset/evidence IDs, host or user identifiers, IPs, credentials, packet data, and raw evidence are not sent. The cache is bounded and tenant-scoped; audit details contain only aggregate status and a context digest. Attack-path registration stores a minimized, tenant-bound summary in `attack_path_analyses` through ordered Alembic revision `8f4c1d2a7b90`. Rollback uses the Alembic downgrade to the prior revision before application rollback; no manual SQL migration runner is used.
