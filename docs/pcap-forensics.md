...
| POST | `/api/v1/evidence/{evidence_id}/integrity/reverify` | Re-verify the immutable manifest and storage metadata | Bearer authentication with `view_audit`; no packet bytes are read or returned |
| POST/GET | `/api/v1/evidence/{evidence_id}/legal-hold` | Place, release, or inspect a tenant-scoped legal hold | `manage_cases` for changes; `read` for inspection; actors are server-derived |
| POST/GET | `/api/v1/evidence/{evidence_id}/retention-decision` and `/retention-history` | Record and review append-only retention decisions | `manage_cases` for decisions; `read` for history; bounded to 100 entries |
| POST | `/api/v1/evidence/{evidence_id}/deletion-request` | Request review of metadata cleanup after retention eligibility | `manage_cases`; active holds and missing eligibility return safe HTTP 409 |
| POST | `/api/v1/evidence/{evidence_id}/deletion-request/{request_id}/decision` | Approve or reject a request without deleting evidence | `view_audit`; approval requires a different actor and rechecks policy gates |
| GET | `/api/v1/evidence/{evidence_id}/privacy-summary` | Return a bounded, privacy-preserving governance summary | Bearer authentication with `read`; no notes, payloads, or storage locators |
...
Manifest verification recomputes the canonical immutable evidence payload used by the shared governance service. Redaction verification rejects raw IP addresses, unpseudonymized DNS names, unexpected payload attributes, and any non-pseudonymized mode. Drill-down returns HTTP 409 through the safe error envelope if either verification fails. Tenant predicates are applied to every lifecycle, manifest, deletion-check, drill-down, and governance query. Integrity re-verification also fails closed for non-metadata storage, a storage locator, retained bytes, a non-zero byte count, or a source-hash mismatch.

The PCAP lifecycle revisions are managed through Alembic. Revision `22d614b2aac8` creates the metadata-only lifecycle record and revision `4f2a6c1e9b77` adds legal holds, retention decisions, and deletion-request approval metadata. Apply them with `alembic upgrade head` after an approved backup and staging validation. Rollback is application-first; if a schema rollback is required, use the reviewed Alembic target `22d614b2aac8` after a metadata export and record the resulting schema version. No rollback path deletes evidence bytes, and this module provides no evidence deletion executor. Tenant filtering is enforced in list, detail, linked-evidence, lifecycle, manifest, deletion-check, drill-down, and governance queries; a valid identifier from another tenant returns HTTP 404 rather than disclosing its existence.

## Test coverage and limitations

`backend/tests/test_pcap.py` covers deterministic hash verification, pseudonymized output verification, bounded flow/DNS/observation summaries, truncated-input rejection, role gating, upload-size limits, linked evidence reads, persistence, and cross-tenant isolation. The lifecycle regression suite covers retention metadata, manifest verification, strict redaction verification, bounded drill-down, quarantine safe failure, lifecycle listing, legal holds, raw-byte absence, and cross-tenant lifecycle isolation. The evidence-governance regression suite covers re-verification, metadata-only storage-provider behavior, privacy summaries, retention history, deletion approval boundaries, and Alembic downgrade/re-upgrade. The slice intentionally does not attempt TLS decryption, file carving, payload replay, exploit detection, arbitrary BPF filtering, live capture, external enrichment, or full protocol dissection. Those capabilities require separate review, resource budgets, and explicit contracts; they are not implied by this module.
